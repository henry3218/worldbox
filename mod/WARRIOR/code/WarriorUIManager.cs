...
{
    // 主UI控制类
    public static class WarriorUIManager
    {
        ...
        public static void Init()
        {
            ...
            UnityEngine.Debug.Log("[武极] Step1: UI initialization begin");

            // 创建标签页和按钮
            var tabIcon = TrySprite("ui/icons/iconTab", "ui/icons/iconBook");
            _tab = TabManager.CreateTab("warrior_mod_tab", "武极寰宇录", "更快捷的模组设置入口", tabIcon);
            _tab.SetLayout(new List<string> { "tools" });

            // 添加设置按钮
            var settingsIcon = TrySprite("ui/icons/iconSetting1", "ui/icons/iconBook");
            ...
            UnityEngine.Debug.Log("[武极] Step2: UI initialization done");
        }
        ...
        private static void OpenNativeModSettings()
        {
            try
            {
                ...
                if (cfg == null)
                {
                    UnityEngine.Debug.LogError("[武极] 未找到 ModConfig（检查 default_config.json / BasicMod<T>.GetConfig() 暴露情况）");
                    return;
                }
                ...
                if (wndType == null) { UnityEngine.Debug.LogError("[武极] 未找到 ModConfigureWindow 类型"); return; }

                var show = wndType.GetMethod("ShowWindow", BindingFlags.Public | BindingFlags.Static);
                if (show == null) { UnityEngine.Debug.LogError("[武极] 未找到 ModConfigureWindow.ShowWindow(...)"); return; }

                show.Invoke(null, new object[] { cfg });
            }
            catch (Exception ex) { UnityEngine.Debug.LogError($"[武极] 打开原生设置窗口失败: {ex}"); }
        }

        // 获取模组配置对象
        private static object TryGetMyModConfig()
        {
            var modType = Type.GetType("PeerlessOverpoweringWarrior.PeerlessOverpoweringWarriorClass")
                       ?? AppDomain.CurrentDomain.GetAssemblies()
                            .SelectMany(a => { try { return a.GetTypes(); } catch { return Array.Empty<Type>(); } })
                            .FirstOrDefault(t => t.Name == "PeerlessOverpoweringWarriorClass");
    ...
    // 移除了WarriorInfoWindow类，不再需要信息窗口功能
}